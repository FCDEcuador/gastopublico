								...
								<div class="row elemento no-gutters">
									...
									<div class="col-sm-7">
										<h6 class="mb-0 ml-sm-3 mb-0 mt-3 mt-sm-0">
											<a href="{!! route('content-article', [$oSecondaryArticle->contentCategory->slug, $oSecondaryArticle->slug]) !!}" class="text-muted public-sidebar" >{{ str_limit($oSecondaryArticle->summary, $limit = 150, $end = '...') }}
											</a>
										</h6>
						...
						<div class="lista-inline text-center flex">
                            @if(is_array($aTags))
								@foreach($aTags as $tag)
									<a role="button" href="{!! route('content-category-with-tag', [$tag['contentCategorySlug'], $tag['tag']]) !!}" name="1" class="tags-Busqueda btn-light pt-3 pb-3 pr-5 pl-5 mr-3 d-inline-block mb-3">{{ $tag['tag'] }}</a>
								@endforeach
							@endif
	                    </ul>
					</div>
